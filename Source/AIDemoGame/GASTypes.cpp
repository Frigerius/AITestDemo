    ...
    if (UAbilitySystemComponent* ASC = UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(Source)) {
        ...
        for (FGASEffectApplicationItem& Item : Items) {
            for (FGASEffectItem& EffectItem : Item.Effects) {
                ...
                EffectItem.GeneratedSpec =
                    FGameplayEffectSpecHandle(new FGameplayEffectSpec(EffectItem.GameplayEffect.GetDefaultObject(), ASC->MakeEffectContext(), Level));
...
TArray<FActiveGameplayEffectHandle> FGASEffectApplicationContainer::ApplyEffectApplicationContainerToTarget(AActor* Target) {
    ...
    if (SourceAbilitySystemComponent == nullptr) {
        UE_LOG(LogTemp, Warning,
               TEXT("ApplyEffectApplicationContainerToTarget called on container with no SourceAbilitySystemComponent. call GeneratEffectSpecs first"));
        return Handles;
    }
        ...
        for (FGASEffectApplicationItem& Item : Items) {
            ...
            for (FGASEffectItem& EffectItem : Item.Effects) {
                ...
                FActiveGameplayEffectHandle NewHandle =
                    ASC->ApplyGameplayEffectSpecToSelf(*EffectItem.GeneratedSpec.Data.Get(), SourceAbilitySystemComponent->ScopedPredictionKey);
